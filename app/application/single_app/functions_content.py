...
def extract_table_file(file_path, file_ext):
    """
    Extract tabular content from CSV or Excel files.

    - CSV: returns a single CSV string.
    - Excel (.xls, .xlsx, .xlsm): returns concatenated CSV for all sheets, each
      preceded by a clear header indicating the sheet name, separated by an
      explicit delimiter for readability in chat.

    This preserves token-efficient CSV while supporting multi-tab spreadsheets.
    """
    try:
        if file_ext == '.csv':
            ...
            for sheet_name in xls.sheet_names:
                ...
                # Header marking the sheet for clarity in chat
                sheet_header = f"### Sheet: {sheet_name}\n"

                # If sheet has data, include CSV; otherwise include header only
                if df is not None and not df.empty:
                    sheet_csv = df.to_csv(index=False)
                    parts.append(sheet_header + sheet_csv)
                else:
                    parts.append(sheet_header + "(no rows)\n")

            # Join sheets with a visible delimiter
            return "\n\n---\n\n".join(parts)
